...
type palette struct {
	...
	// beside the warm one reads as a different FACT. Owner-approved as a pair
	// (2026-08); nothing draws it yet — one lit span at a time is the recorded
	// owner invariant, held by tint_multi_guard_test.go until asked.
...
var lightPalette = palette{
	...
	// 4.2:1 on it (theme_contrast_test.go pins >= 3.0). Approved by the owner
...
var darkPalette = palette{
	...
	// theme_contrast_test.go). Owner-approved with #C7DBF5; unreachable until
	// asked (tintMulti).
...
// finishes the June change rather than making a new one (owner, 21 Aug 2026).
