...
// (docs/NOTES_SCRAPBOOK.md, the tint table). With the rule copied six times,
...
const (
	...
	// tintMulti — MORE THAN ONE note covers this verse (docs/NOTES_SCRAPBOOK.md,
	// the tint table). Fully wired through the tables below and the palette
	// (HighlightMulti, theme.go — owner-approved as a hue-separated pair, light
	// #C7DBF5 / dark #2E3E5C), and deliberately UNREACHABLE: chapterTint never
	// returns it, because one lit span at a time is the recorded owner
	// invariant. tint_multi_guard_test.go holds that line — wiring a code path
	// to this constant is an owner decision, not a side effect. When it is
...
// the owner caught it on John 11:25, and there is now no way to write a wash arm
