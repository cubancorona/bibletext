...
func TestLegacyStoresMigrateOnFirstRead(t *testing.T) {
	p := newNotePrefs()
	p.m[prefLegacySharedNotes] = `[{"v":"web","b":"John","c":3,"lo":16,"t":"from a friend","ts":100,"m":true,"sn":"Anna","sid":"9a3f01c7528e"},` +
		`{"v":"bsb","b":"Psalms","c":23,"t":"another","ts":200}]`
	p.m[prefLegacyMyNotes] = `[{"v":"web","b":"Romans","c":8,"lo":28,"t":"sent to Dad","ts":300,"me":true}]`
	...
	if friend.Kind != noteKindReceived || !friend.Minimized || friend.Received != 100 ||
		friend.SenderName != "Anna" || friend.SenderID != "9a3f01c7528e" || friend.VerseLo != 16 {
