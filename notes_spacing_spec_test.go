...
// fails on the dev machine and in CI on EVERY platform's numbers at once
	...
	for _, c := range []check{
		{
			...
			required: map[string]string{
				...
				// silently off-spec, and the parser could not see it (refuter
				// finding 4 — the exact quantity the owner asked to be the same).
...
// docs/NOTES_SCRAPBOOK.md is told to expect.
	...
	for _, tc := range []struct {
		...
		if tc.got != tc.want {
			t.Errorf("noteMetrics().%s = %v, want %v — if this is a deliberate change, "+
				"docs/NOTES_SCRAPBOOK.md's per-platform table has to move with it",
