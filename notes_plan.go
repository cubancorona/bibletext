...
// sender typed); S9 is the one reviewed native change that unbends it.
//
...
//     no longer makes the rest of the set invisible (S8 review finding). An
...
//     and an empty sender bubble must never render (task 3's shape: within
...
//     review finding; nothing is stored). Also forced for unplaced-only.
//
