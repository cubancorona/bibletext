...
func applyShareTarget(state *AppState, t ShareTarget) {
	...
	if notesFeatureOn(state) {
		...
		// just put that stored note in place, and it stays (emulator-caught: a
		...
		if t.Note != "" {
			...
			if remembered {
				...
				// owner met that behaviour on a real device ("does not expand
				...
				// reader has. (Owner-reported, 2026-08-18.)
				if stored.Minimized {
				...
				// Review measured the regression: pre-S7 the re-arrival relit
...
// authoritative for what is filed, exactly as the S4 commit said they would
// become.)
func noteStorageTarget(t ShareTarget) ShareTarget {
	...
	if t.NoteLo > 0 {
		...
		// verse span, so only a hand-built wire reaches this; review found the
