...
func nkjvShapedBible(t *testing.T) *BibleData {
	t.Helper()
	psalm, _, _, _, err := decodeAPIBibleChapter(json.RawMessage(psalmChapterContent), "Psalms", 46)
	if err != nil {
		t.Fatal(err)
	}
	john, _, _, _, err := decodeAPIBibleChapter(json.RawMessage(proseChapterContent), "John", 3)
	if err != nil {
		t.Fatal(err)
	}
	return &BibleData{
