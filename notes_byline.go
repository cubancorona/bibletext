...
// docs/NOTES_SCRAPBOOK.md "Identity — the decision record").
//
...
// and stored since the record codec landed — and the owner's directive is to
...
// senderNamesEnabled is the build flag the owner flips when the name field
...
func senderByline(n StoredNote) string {
	if n.Kind == noteKindMine {
		// Own notes are never drawn in the reading text (owner directive), so
...
// marks and the BOM by name, and review defeated it with U+200B: a name
...
func senderNameRefused(name string) bool {
	...
	// "BibleText Security", "bible text", "Anna BibleText" — anything that
