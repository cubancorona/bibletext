...
# patched copy, run the patch's own tests, and hold cmd/desktop/FyneApp.toml
...
if ! grep -q '^\[CanOpen\]$' cmd/desktop/FyneApp.toml \
   || ! grep -q '^MimeTypes = "x-scheme-handler/bibletext;"$' cmd/desktop/FyneApp.toml; then
  echo "ERROR: cmd/desktop/FyneApp.toml no longer registers x-scheme-handler/bibletext under [CanOpen]" >&2
  exit 1
fi
