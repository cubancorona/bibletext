...
func buildSidebar(state *AppState) fyne.CanvasObject {
	...
	// --- Ask-AI search (natural-language passage finder). Results replace the reading
	...
	findCaption := caption("Keyword, or a reference like John 3:16.")
	askCaption := caption("These results are generated by AI and may be inaccurate or incomplete. Always read each passage in its full context and confirm it in Scripture.")
	applyMode := func() {
		if state.aiSearchMode {
			fieldHost.Objects = []fyne.CanvasObject{askRow}
			captionHost.Objects = []fyne.CanvasObject{askCaption}
		} else {
			fieldHost.Objects = []fyne.CanvasObject{searchRow}
			captionHost.Objects = []fyne.CanvasObject{findCaption}
		}
		fieldHost.Refresh()
		captionHost.Refresh()
	}
