...
func TestCachePathForVersion(t *testing.T) {
	...
	wantWEB := filepath.Join(dir, "bibletext-web-v5.json")
	...
	// BSB carries a cacheEpoch (its decoder has changed five times), so its
	// cache path is versioned and stale pre-epoch caches are bypassed.
	wantBSB := filepath.Join(dir, "bibletext-bsb-v6.json")
...
func TestPurgeSupersededCaches(t *testing.T) {
	...
	staleV2 := filepath.Join(dir, "bibletext-bsb-v5.json") // v5 (superseded)
	current := filepath.Join(dir, "bibletext-bsb-v6.json") // v6 (active)
