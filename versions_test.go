...
func TestCachePathForVersion(t *testing.T) {
	...
	wantWEB := filepath.Join(dir, "bibletext-web-v6.json")
	...
	// BSB carries a cacheEpoch (its decoder has changed six times), so its
	// cache path is versioned and stale pre-epoch caches are bypassed.
	wantBSB := filepath.Join(dir, "bibletext-bsb-v7.json")
...
func TestPurgeSupersededCaches(t *testing.T) {
	...
	staleV2 := filepath.Join(dir, "bibletext-bsb-v6.json") // v6 (superseded)
	current := filepath.Join(dir, "bibletext-bsb-v7.json") // v7 (active)
	...
	if _, err := os.Stat(staleV2); !os.IsNotExist(err) {
		t.Errorf("stale v6 cache should have been removed (err=%v)", err)
	}
...
func TestVersionCacheIsCurrent(t *testing.T) {
	...
	current := filepath.Join(dir, "bibletext-web-v6.json")
	if err := saveBibleToCache(current, fullValidBible(), currentUTCTime); err != nil {
		t.Fatal(err)
	}
	if !versionCacheIsCurrent(web) {
		t.Error("the v6 file is web's current epoch")
	}
