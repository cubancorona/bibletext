...
func TestDeleteReachesTheNoteOnScreenNotAStaleOne(t *testing.T) {
	...
	// MUM's note is stored under the BSB only. The reader is in the WEB, where it
	// FOLLOWS — so the derive sets NoteVersionID to "bsb", correctly.
	saveNote(appPrefs(), SharedNote{VersionID: "bsb", Book: "John", Chapter: 3, VerseLo: 16, Text: "MUM's note"})
	...
	if !mumSurvives {
		t.Error("REPRODUCED: deleting the friend's note destroyed MUM's note instead")
	}
	if friendSurvives {
