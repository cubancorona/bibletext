...
func TestDeleteReachesTheNoteOnScreenNotAStaleOne(t *testing.T) {
	...
	// MUM's note is stored under the BSB only. The reader is in the WEB, where
	// it FOLLOWS — so the derive hands the mirror MUM's note's identity.
	mum, ok := addNote(appPrefs(), StoredNote{Kind: noteKindReceived, VersionID: "bsb", Book: "John", Chapter: 3, VerseLo: 16, Text: "MUM's note"})
	if !ok {
		t.Fatal("precondition: MUM's note was not stored")
	}
	applyNoteForCurrentChapter(st)
	if st.NoteID != mum.ID {
		t.Fatalf("precondition: the bsb note should have followed, got id %d want %d", st.NoteID, mum.ID)
	}
	...
	if _, mumSurvives := findStoredNote(appPrefs(), "bsb", "John", 3); !mumSurvives {
		t.Error("deleting the friend's note destroyed MUM's note instead")
	}
