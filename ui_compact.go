...
//
// Owner's reason, and the one to weigh before splitting it again: "let's
...
func buildCompactUI(state *AppState) fyne.CanvasObject {
	...
	// a preview (compactNavRail) while the owner decides whether a desktop
...
func buildMobileTabBar(state *AppState) fyne.CanvasObject {
	...
	// The owner tried the pill on both and kept it only where it belongs.
	//
...
// (owner: "you've got those wide horizontal margins and no vertical margin
...
const (
	...
	// The pill's inner margins, horizontal ≈ 3 × vertical (owner). A capsule
...
//
// WHY IT IS NOT A LIST ANY MORE (owner, 21 Aug 2026: "the books list is a little
...
func (c *tabCell) CreateRenderer() fyne.WidgetRenderer {
	...
	// part of the same control (owner, on both form factors). A custom padded
...
//
// The floating pill was the tablet's dress until 21 Aug 2026. The owner compared
