...
.wrap{max-width:38rem; margin:0 auto; padding:1rem 1.6rem 2rem}
.top{
...
.vers{display:flex; gap:.3rem}
.goto{
  margin-left:auto; color:var(--accent); text-decoration:none; font-size:.75rem;
  border:1px solid var(--border); border-radius:999px; padding:.15rem .6rem;
}
.goto:hover{border-color:var(--accent)}
...
.chapbar{display:flex; align-items:baseline; gap:.75rem}
.chapnav{margin-left:auto; display:flex; gap:.15rem; white-space:nowrap}
.arrow{
  display:inline-block; min-width:2rem; text-align:center;
  padding:.15rem .45rem; border-radius:8px; font-size:1.25rem; line-height:1.4;
  color:var(--accent); text-decoration:none;
}
.arrow:hover{background:var(--hl)}
.arrow.off{color:var(--muted); opacity:.35; pointer-events:none}
...
.foot{max-width:38rem; margin:0 auto; padding:0 1.6rem 2.5rem; text-align:center}
