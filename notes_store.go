...
// the scrapbook store, once. Best-effort by design (the owner's notes are the
...
func applyNoteForCurrentChapter(state *AppState) {
	...
	// YOUR OWN NOTE, when you asked to see it. It is not in plan.Notes and so
...
// anchor (docs/NOTES_SCRAPBOOK.md, hard case 11).
...
func hideCurrentNote(state *AppState) {
	...
	// because you asked to see it, and only until you navigate away, so a
...
func dropCurrentNote(state *AppState) {
	...
	// unconfirmed, in one tap, from a card that appeared because you asked to
