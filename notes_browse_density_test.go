...
// The browser's DENSITY (owner, 2026-08-19: "the notes browser takes up far
...
// The owner's target, measured honestly: at least TWICE the rows per screen.
...
func TestBrowserShowsTwiceTheRowsPerScreen(t *testing.T) {
	...
	typical := StoredNote{Kind: noteKindReceived, VersionID: "web", Book: "Psalms",
		Chapter: 23, VerseLo: 1, Text: "Praying for you this morning.", Received: 1_700_000_000}
	...
	if rows := int(phoneViewport / h); rows < 10 {
		t.Errorf("a typical row measures %vpt — %d rows per %dpt screen, want at least 10 "+
			"(twice the ~5 the owner was seeing)", h, rows, phoneViewport)
	}
}
