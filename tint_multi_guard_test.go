...
// tintMulti IS WIRED AND MUST NOT BE REACHABLE (S12, docs/NOTES_SCRAPBOOK.md).
//
...
// and nothing else. Until the owner asks for it, ONE LIT SPAN AT A TIME is the
// recorded invariant — a chapter never carries two washes — and this test is
// what keeps a refactor from breaking that by accident: chapterTint is the one
// tint source every renderer consults (tint.go), so proving tintMulti cannot
// flow out of tint.go's answer proves no surface can draw it.
//
	...
	for _, u := range uses {
		...
		if !allowed[u] {
			t.Errorf("tintMulti is referenced at %s — outside its declaration and its three "+
				...
				"draw a second wash until the owner asks. If this is that decision, move the "+
				"invariant deliberately: widen chapterTint, update this guard, and re-judge "+
				"the palette on a rendered page (docs/NOTES_SCRAPBOOK.md, S12).", u)
		}
	}
	if !declared {
