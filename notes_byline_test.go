...
// deliberate owner decision that lands together with this line changing.
func TestSenderNamesAreDormant(t *testing.T) {
	if senderNamesEnabled {
		t.Fatal("senderNamesEnabled is on — sender names would display; this is an owner decision " +
			"(docs/NOTES_SCRAPBOOK.md, Identity). If it is intended, update this test and re-judge " +
			"every byline surface.")
	}
	n := StoredNote{Kind: noteKindReceived, SenderName: "Anna"}
	...
	if got := noteByline(StoredNote{Kind: noteKindMine, SenderName: "Anna"}); got != "From you" {
...
func TestSenderNameDisplayRules(t *testing.T) {
	cases := []struct {
		...
		{"plain", "Anna", iso("Anna")},
		{"spaced", "Anna Rose", iso("Anna Rose")},
		...
		{"newline", "Anna\nRose", iso("Anna Rose")},
		{"crlf and tabs", "Anna\r\n\tRose", iso("Anna Rose")},
		{"run of blanks", "Anna    Rose", iso("Anna Rose")},

		// Bidi steering INSIDE a name is stripped before our isolates go on.
		{"rlo stripped", "‮anna", iso("anna")},
		{"isolates stripped", "⁦anna⁩", iso("anna")},
		...
		{"trailing dot ok", "Anna B.", iso("Anna B.")},
	}
	for _, c := range cases {
...
func TestSenderBylineComposition(t *testing.T) {
	...
	n := StoredNote{Kind: noteKindReceived, SenderName: "Anna"}
	if got := "Note from " + senderNameWithFlag(n, true); got != "Note from "+iso("Anna") {
...
// The two dormant-path bypasses review found, pinned so they stay dead.
...
// before this pin (review finding).
