...
func TestBrowserTapAlwaysLandsOpen(t *testing.T) {
	...
	t.Run("your own note answers with its verses lit", func(t *testing.T) {
		...
		// AMENDED 20 Aug 2026 (owner). This block used to assert the opposite:
		...
		if st.ActiveNote != mine.Text {
			t.Errorf("your own note must be drawn on the passage you asked to see it on: "+
...
func TestOwnNoteTapDoesNotSuppressAFriendsNote(t *testing.T) {
	...
	friend, ok := addNote(appPrefs(), StoredNote{Kind: noteKindReceived, VersionID: "web",
		Book: "Psalms", Chapter: 23, VerseLo: 4, Text: "thinking of you"})
	if !ok {
...
// YOUR OWN NOTE IS EPHEMERAL: shown because you asked, gone when you move on.
...
func TestOwnNoteHidesAgainOnNavigation(t *testing.T) {
	...
	openNote(st, mine)
	if st.ActiveNote != mine.Text {
		t.Fatalf("your own note was not drawn when you asked for it: %q", st.ActiveNote)
	}
...
//
// This is the whole round trip the owner described: "when I send a note it goes
...
func TestOwnLinkComesHomeAsYours(t *testing.T) {
	...
	mine, ok := addNote(appPrefs(), StoredNote{Kind: noteKindMine, VersionID: "web",
		Book: "Psalms", Chapter: 23, VerseLo: 1, Text: "sent to Dad", Nonce: nonce})
	if !ok {
		t.Fatal("your note was not stored")
	}

	// The link you sent, parsed back exactly as tapping it would.
	url := ShareLinkURLWithNoteNonce("web", "Psalms", 23, 1, 1, "sent to Dad", nonce)
...
// nonce exists to prevent: short notes repeat ("Amen", "praying for you"), and
...
func TestTappingYourOwnLinkDrawsYourOwnNote(t *testing.T) {
	...
	mine, ok := addNote(appPrefs(), StoredNote{Kind: noteKindMine, VersionID: "web",
		Book: "John", Chapter: 3, VerseLo: 16, Text: "sent to Dad", Nonce: nonce})
	if !ok {
		t.Fatal("your note was not stored")
	}

	st := planTestState(t)
	url := ShareLinkURLWithNoteNonce("web", "John", 3, 16, 16, "sent to Dad", nonce)
...
func TestReadingPageVerbsCannotDestroyYourOwnNote(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mine, ok := addNote(appPrefs(), StoredNote{Kind: noteKindMine, VersionID: "web",
				Book: "John", Chapter: 3, VerseLo: 16, Text: "sent to Dad"})
			if !ok {
				t.Fatal("your note was not stored")
			}
			st := planTestState(t)
...
func TestReadingPageVerbsStillWorkOnAReceivedNote(t *testing.T) {
	...
	theirs, ok := addNote(appPrefs(), StoredNote{Kind: noteKindReceived, VersionID: "web",
		Book: "John", Chapter: 3, VerseLo: 16, Text: "thinking of you"})
	if !ok {
...
// THE ROW'S DELETE CONTROL COSTS NO HEIGHT (owner: "does not increase vertical
// height ... at all").
//
// The browser's density is a thing the owner has fought for twice, so this
...
func TestNoteRowTrashCostsNoHeight(t *testing.T) {
	...
	for _, tc := range []struct{ name, text string }{
		...
		{"a one-line note", "thinking of you today"},
		{"a wrapping note", "thinking of you today and praying that this week is gentler " +
