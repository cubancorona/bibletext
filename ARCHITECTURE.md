...
| `webbe_audio.go` | WEB-Catholic Greek-books URLs (eBible.org WEBBE synthetic narration, 150 chapters: the deuterocanon, the Greek Esther, Daniel 3/13/14) |
...
| `reporter_ios.go` / `reporter_android.go` / `reporter_macos.go` / `reporter_other.go` | `reporterLayoutActive()` — whether the native pane is on the book page (the U.S. Reports set), asked of the reading page by the pane's width (`reading_page.go`, `reading_page_width.go`), never by device — save for a push made before any width is known, which takes the device's resting page (`readingUnsizedPage`); false on Windows and Linux, whose styled pane lays its page out from its own width |
| `device_ios.go` / `device_android.go` / `device_other.go` | `deviceIsTablet()` — UIKit interface idiom on iOS; sw600dp-style smallest-dimension test on Android (`isTabletDimensions`, live canvas); false on desktop. Tablet identity plus live orientation chooses bar versus rail. The same files state whether a platform has the phone-landscape presentation and how it keeps the place across a rotation (`phoneLandscapeReadingSupported`, `rotationRestoreNeeded`) |
| `textsize.go` | Settings → Reading → Text size: the persisted scale (1.0/1.15/1.3) the scripture body renders at on every platform |
| `chapter_header_mobile.go` | The compact mobile chapter toolbar shared by both native reading views; untagged although only those two panes call it, so the host suite lays out and taps the real header (`chapter_header_cover_test.go`). The row draws the audio control beneath its neighbours while it is the speaker and above them while the card is open, by moving it between two fixed places in the row (`audioPlaceLayout`), hides the small controls whose glyphs the open card touches (`cardCover`, `audio_button.go`), and stops the open card at its right edge so the ✕ stays on the screen (`phoneAudioCellLayout`) |
...
| `audio_button.go` | The reading-header audio control: collapsed speaker → expanded mini-player (source indicator + ±15s skip + play/pause), self-refreshing host (no pane rebuild). The open card is a pop-up that may cover neighbouring header controls until its ✕ closes it; `tapShield` makes it swallow every tap and touch across its rectangle, and `cardCover` hides, in both headers, each small control whose glyph it touches (and the full-screen button when it touches its focus highlight), in a slot that keeps its size so nothing moves |
...
| `reading_fyne.go` | `ios \|\| !darwin` | `readingScrollArea`: dispatches the styled pane on Windows/Linux, else the legacy `chapterText` Entry pane (the desktop burn-in fallback). Compiled but unreachable on iOS and Android — the mobile layouts call `buildReadingViewMobile`, and Android's bridge-absent fallback is `reading_mobile.go`'s RichText pane |
...
A reading-header control plays the current chapter as a recorded **narration** — human
apart from the WEB-Catholic's Greek books, which use a synthetic voice — or on-device
**read-aloud** (text-to-speech). Recorded narration plays on **every
...
The **WEB-Catholic** carries a SECOND recording after Williams in preference order: a
public-domain synthetic narration of its Greek books ([webbe_audio.go](webbe_audio.go),
150 chapters — the deuterocanon, the Greek Esther and Daniel 3/13/14, which are text
Williams never read). The two are complementary, so a chapter still offers at most one
recording; everything else (other versions, unrecorded chapters) falls back to TTS of
the on-screen verses (`chapterSpeechText`). What the app advertises and what the host holds can drift — BSB Daniel 7-10 and Romans 7
were offered for weeks with no MP3 behind them — so `TestLiveAudioAssetsExist`
(`BIBLETEXT_CHECK_AUDIO=1`) reconciles every advertised chapter against the release
assets; run it after uploading a recording. The recordings stream from the project's own audio
...
- Audio narration: **Berean Standard Bible** recording (Barry Hays) — CC0;
  **World English Bible** recording (David Williams) — public domain; the
  **WEB-Catholic** Greek books' **synthetic** narration (eBible.org WEBBE) — public
  domain; all streamed
