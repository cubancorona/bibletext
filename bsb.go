...
// fetch is decoded into BibleData and cached as bibletext-bsb.json like any other
// version (see loadVersionData / cachePathForVersion).
...
//
// The two element kinds carry spacing differently, so they can't all just be
// space-joined: helloao bakes the surrounding spaces INTO plain-string runs (a
// footnote that splits a sentence yields runs like "...nations'" + "? But you..."
// that must abut, and a run after a mid-word footnote starts with its own leading
// space), whereas poetry/descriptive clauses carry NO surrounding spaces. So
// plain strings are concatenated verbatim (space-joining them would wrongly insert
// a space before closing punctuation/quotes after a footnote), and a single space
// is inserted only at a boundary that involves a clause — between consecutive
// clauses, and between a clause and the prose around it (e.g. "And the man said:"
// + clause). The final strings.Fields pass collapses any redundant whitespace.
func bsbVerseText(content []json.RawMessage) string {
	...
	for _, node := range content {
		...
		if err := json.Unmarshal(node, &s); err == nil {
			...
			if b.Len() > 0 && prevWasClause {
				b.WriteByte(' ') // prose abutting a preceding poetry clause
			}
			b.WriteString(s)
			prevWasClause = false
			continue
		...
		if err := json.Unmarshal(node, &obj); err == nil && obj.Text != nil {
			...
			if b.Len() > 0 {
				b.WriteByte(' ') // clauses carry no surrounding spaces
			}
			b.WriteString(*obj.Text)
			prevWasClause = true
			continue
		}
		// noteId / lineBreak: contribute nothing, leave spacing unchanged.
	}
