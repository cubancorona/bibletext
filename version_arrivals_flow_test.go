...
// arrivalWorld is the disk the journeys walk and the source the link's
// translation is fetched from. A landing rewrites the disk, so every journey
// puts it back before it replays.
...
// reset puts the link's translation's disk back to the world's shape and the
// network back up. A landing writes the current edition and purges the
// previous one, and every journey starts from the beginning.
...
func (e arrivalEvent) String() string {
	return [...]string{"link-names-other", "fetch-fails", "fetch-fails-previous-serves", "fetch-lands", "reader-picks-it", "reader-picks-other"}[e]
}
...
// a goroutine deterministically. Where production is synchronous and cannot
// reach the network, the real function runs.
func (e arrivalEvent) apply(t *testing.T, w *arrivalWorld, st *AppState, told *bool) {
	t.Helper()
	switch e {
	case arLinkNamesOther:
		if st.CurrentVersion == arrivalLinkVersion || st.loading() {
			// switchToLinkVersion's own guards. While a load runs the link
			// parks behind it, and in these journeys the only load that runs
			// is this translation's, so the park is the one already there.
			return
		}
		if _, inMem := st.loadedVersions[arrivalLinkVersion]; inMem {
			...
			applyShareTarget(st, ShareTarget{VersionID: arrivalLinkVersion, Book: "John", Chapter: 1, VerseLo: 16})
			return
		}
	...
	case arFetchFails, arFetchFailsPreviousServes:
		...
		v, _ := versionByID(arrivalLinkVersion)
...
var knownArrivalIncoherent = []pinnedArrivalDefect{
	{
		...
		covers: func(_, now arrivalFacts, _ arrivalEvent) bool {
			return now.stuck && now.current == arrivalLinkVersion && now.edition == "previous"
		},
	},
}
...
func checkArrivalInvariants(prev, now arrivalFacts, ev arrivalEvent) []string {
	...
	if now.edition == "previous" {
		...
		if v.Name == "" || !strings.Contains(now.notice, v.Name) {
...
func TestArrivalJourneysKeepTheirPromise(t *testing.T) {
	...
	report := func(disk arrivalDisk, path []arrivalEvent, prev, now arrivalFacts, e arrivalEvent, violations []string) {
		for _, bad := range violations {
			...
			// Keyed on the VIOLATION and the disk, not the path: thousands of
			// journeys reach the same broken promise by hundreds of routes,
			// and a report that lists them all buries the handful of distinct
			// facts. The SHORTEST route is kept, because that is the one to
			// read.
			key := fmt.Sprintf("%s: %s: %s", disk, e, bad)
	...
	events := []arrivalEvent{arLinkNamesOther, arFetchFails, arFetchFailsPreviousServes, arFetchLands, arReaderPicksIt, arReaderPicksOther}

	for _, disk := range []arrivalDisk{arDiskCurrent, arDiskPrevious, arDiskNone} {
		w.disk = disk
		began, worldJourneys, worldSteps := time.Now(), journeys, steps
		...
		sawPrevious, sawTold, sawCurrentLink := false, false, false
		...
		walk = func(path []arrivalEvent, depth int) {
			...
			for _, ev := range events {
				...
				// disk. AppState carries an atomic, so it can never be copied
				// to fork a journey — every branch is replayed from the
				// beginning instead.
				w.reset(t)
				...
				for _, e := range next {
					...
					report(disk, next, prev, now, e, checkArrivalInvariants(prev, now, e))
				...
				ends[pathString(next)] = arrivalEnd{path: next, facts: prev}
				walk(next, depth-1)
			}
		}
		...
		for _, s := range checkArrivalLiveness(ends, arrivalDepth) {
			...
			report(disk, s.path, s.facts, s.facts, last,
		...
		case arDiskCurrent:
			if sawPrevious || sawTold {
				t.Errorf("control: %s put a previous edition on screen or failed a load, and with the current edition on disk neither can happen", disk)
			}
		case arDiskPrevious:
			if !sawPrevious || sawTold {
				t.Errorf("control: %s never served the previous edition, or failed a load it could not fail, so the arm it exists for was not walked", disk)
			}
		case arDiskNone:
			if sawPrevious || !sawTold {
				t.Errorf("control: %s served a previous edition it does not have, or never failed a load", disk)
			}
		}
		if !sawCurrentLink {
			t.Errorf("control: %s never put %s's current edition on screen, so nothing could count as a way back from a previous one", disk, arrivalLinkVersion)
		}
		t.Logf("%s: %d journeys / %d steps in %v", disk, journeys-worldJourneys, steps-worldSteps, time.Since(began).Round(time.Millisecond))
	}
	...
	t.Logf("%d journeys / %d steps walked over three disks; stuck on a previous edition: %d; %d pinned incoherent states reached", journeys, steps, stuck, len(seen))
}
...
func TestTheArrivalInvariantsCanActuallyFail(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"A-C", arrivalFacts{current: link.ID, edition: "current", parked: true, arrivalOwed: true}},
		{"A-D", arrivalFacts{current: link.ID, edition: "previous", notice: said}},
		{"A-D", arrivalFacts{current: link.ID, edition: "current", stale: true, notice: said}},
		{"A-E", arrivalFacts{current: link.ID, edition: "previous", stale: true}},
	} {
		got := checkArrivalInvariants(tc.now, tc.now, arLinkNamesOther)
		fired := false
		for _, bad := range got {
	...
	// a park with its load still running, and a previous edition that is
	// recorded and said.
	for _, quiet := range []arrivalFacts{
		{current: link.ID, edition: "current", parked: true, arrivalOwed: true, loading: true},
		{current: link.ID, edition: "previous", stale: true, notice: said},
	} {
		if got := checkArrivalInvariants(quiet, quiet, arLinkNamesOther); len(got) > 0 {
			t.Errorf("a coherent state was reported: %s: %q", quiet, got)
		}
	}
