...
> **Status.** Sections marked **[OBSERVED]** describe a legacy code snapshot and
> retain its `file:line` evidence. **S5 replaced the passage-keyed store**:
...
> that cite the old functions are historical. **S7 made the model plural**:
> `buildChapterPlan`
...
> PLAN (zero Open) while writing nothing. **S8 made the reading view plural**:
> the Fyne banner
...
>
> This document preserves legacy state names, transition evidence, and defect
> identifiers. [`NOTES_SPEC.md`](NOTES_SPEC.md) defines the current durable
> architecture.
...
| **the key** | `version\|book\|chapter` | `noteKey` (`:53-55`). **Not changing** (settled). Consequence: at most ONE note per translation per chapter — two people sending notes on the same chapter in the same translation still overwrite each other (`:15-19`). |
...
named. Those are the two target presentation changes.
...
collapsed. This is a legal target state: an explicit minimize must be
...
READING VIEW'S bubble, with the version label OUTSIDE it.
...
- **N3 — No silent substitution.** The text in the bubble must not change to a
  ...
  and the harness checks the invariant over the plan (an open note after
...
|---|---|---|---|---|
| ~~X1~~ | ~~Delete kills the wrong note~~ | **FIXED** `69d3f4ab3` | 0 | — |
| ~~X2~~ | ~~Hide is a silent no-op~~ | **FIXED** `69d3f4ab3` | 0 | — |
...
### X1 and X2 — FIXED 2026-08-15 (`69d3f4ab3`)
...
harness checks N3 over the plan and the predicate covers zero cells.
...
**Evidence.** Enumeration at `69d3f4ab3`: 4 cells, `N3-substituted`,
...
again on the next navigation; Hide was the same silent no-op. `69d3f4ab3` made
...
Making the clear conditional exposed four cells where a note-bearing link had
lit its own verses as `hlLinkSpan`, which Delete then walked past. The resolution
is semantic: `hlLinkSpan` means a link range with no note; when a link carries a
note, the note owns the mark.
...
**Why it is wrong.** The derive preserves a highlight that belongs to another
action (`notes_store.go:327-330`). Both verbs then call
...
The legacy model has three measurable structural mismatches.

### 1. The arity is wrong, and no guard can fix an arity

`AppState` models exactly ONE note (`state.go:74-90`). The required cardinality
is a set. The temporary presentation cap permits at most one expanded note,
including none, and must remain removable without persistent residue.
...
//
// TEMPORARY (2026-08): at most one expanded at a time. Zero expanded is
...
`version|book|chapter`, unchanged, as settled. Everything above works
...
| browser | its own row layout, `notes_browse.go:390-424` | the SAME bubble builder as the reading view, with the version label outside it **[INTENDED]** — one builder is what stops the two surfaces disagreeing |
