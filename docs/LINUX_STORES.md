...
| Executable | `bibletext` on every channel, the tarball included since 19 September 2026 | `fyne package` names it after `cmd/bibletext` unless `--executable` says otherwise; the tarball shipped `desktop` until then, which put a generic binary in the reader's PATH and named the app `desktop` in their volume control |
