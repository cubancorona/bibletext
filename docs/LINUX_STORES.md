...
| Executable | `bibletext` on every channel, the tarball included since 19 September 2026 | `fyne package` names it after `cmd/desktop` unless `--executable` says otherwise; the tarball shipped `desktop` until then, which put a generic binary in the reader's PATH and named the app `desktop` in their volume control |
...
| AppImage name | `BibleText-x86_64.AppImage` and `BibleText-aarch64.AppImage`, each plus `.zsync`, constant per architecture | the site links `/releases/latest/download/` by name; the version travels inside as `X-AppImage-Version` |
...
| Architectures | **x86_64 and arm64.** The tarball and the snap ship both. The AppImage ships both too, since 19 September 2026: `scripts/build-appimage.sh` pins an appimagetool and a type-2 runtime per architecture, verified with the x86_64 hashes as controls. `flatpak/flathub.json` names **both x86_64 and aarch64** as of 19 September 2026, after the `aarch64` leg of `linux-stores.yml` built and smoked green. | The former reason read "the toolkit selects GLES on arm64 and the repository has never built or smoked Linux/arm64" — which was a PROOF gap recorded as an architecture exclusion, so it read as settled rather than untested. Both halves were answered on 18 September 2026 on an arm64 machine: the executable builds clean with no new patches and passes `scripts/smoke-linux-launch.sh` (launch, `bibletext:` handoff, single instance); the snap packs in LXD, installs, exports its desktop entry with `%u`, and its ALSA layout resolves **inside the confinement**. That last one is why this mattered: the layout binds a real path, so an arm64 snap carrying the x86_64 triplet builds, installs and launches perfectly and has no narration audio. |
