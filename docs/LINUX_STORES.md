...
| Executable | `bibletext` on every channel, the tarball included since 19 September 2026 | `fyne package` names the binary after its source directory. That was `cmd/desktop` until 19 September 2026, so the tarball shipped a binary called `desktop` -- a generic command in the reader's PATH, and the name their volume control showed while narration played. The directory is now `cmd/bibletext` |
