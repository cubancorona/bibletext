<!-- A design plan. WHAT IS LANDED so far:
     ...
     <pending>  step 4, Tail: one appended flag on all three native ABIs. iOS and
                ...
                SetNote; since 12 Sep 2026 every band formula reads it through
                the drawn shape's extra — btIOSNoteShapeExtra /
                btMacNoteShapeExtra, 0 for a pill — so a collapsed sticker no
                longer reserves the card's tail slot; the outline's
...
The bug is that the tail's depth is gated on `btIOSNotePill()` — "is this collapsed" — when the question is "does this note point at a passage". Push a `Tail` bool and patch one branch and you have fixed the instance while leaving five copies of a locally-derived conditional standing. So the *shape* is taken — the term is resolved once, outside the formula — without the geometry push, which `notes_bubble.go` already rejected for reasons that still hold. The native resolves `gNoteShapeExtra = gNoteTail ? kNoteTail : 0` **once, at push time**. The numbers stay compile-time and parsed; only the *decision* crosses the wire. (As built, and corrected on 12 Sep 2026: the band formulae read that scalar through the drawn shape's extra, `btIOSNoteShapeExtra()` / `btMacNoteShapeExtra()`, which is 0 for a pill — a pill draws no tail whatever its anchor says, and reading the raw scalar reserved the card's tail slot, empty, under every collapsed sticker. The `banned` fragment the plan proposed for `btIOSNotePill() ?` inside the install and layout functions was never added; the spec test pins the shape-extra spellings instead.)
...
- **macOS's bottom-up placement** off the used rect, which keeps `GapBelow` — the pinned invariant — exact. (`btMacNoteTopGap`'s larger reservation, once listed here as a documented residual, is gone: since 12 Sep 2026 it returns the spec's `GapAbove` like every other surface.)
