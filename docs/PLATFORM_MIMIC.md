...
Dev-build-only mode (owner task #43): the macOS **dev** build follows the
...
- **Linux font inventory.** Mimic=linux drops to the embedded Gelasio because
  this Mac has no DejaVu Serif at the Linux paths — the **Linux face is
...
- A bundled **DejaVu Serif** embed for mimic=linux (the ~350 KB dev-only embed
  the audit offered). Gelasio approximation + this caveat was chosen instead.
