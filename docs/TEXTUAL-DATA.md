...
same `\wj` markers — **[measured]** WEB 2,054 verses (2,053 with every span
located), WEBC 2,054 (2,052). Five verses per edition do not match eBible's
revision at all, carry no entry, and fall back to the whole-verse answer.

The old verse-granular behaviour is what every pane still does for any verse
without span data, and it is what the switch restores. Its defect, for the
record: **[measured]** 657 of 2,059 red verses (32%) contained text outside the
`\wj` span, and about 79 put another speaker's words in red.
...
Four reading panes render scripture, in three markup dialects plus one widget
tree, so a rendering rule has to be implemented four times. All four consult the
same `redLetterRuns` / `redLetterSpansFor`:
...
The styled desktop pane colours whole TOKENS, the other three split at rune
...
# red letters — the BSB, derived (no API calls; needs the WEB USFM for candidates)
python3 scripts/gen-bsb-redletter.py --usfm <unpacked eng-web_usfm.zip> \
    --bsb ~/Library/Caches/bibletext/bibletext-bsb-v3.json
...
go test -run 'TestMapVerse|TestVerseExistsIn|TestWordsOfChrist|TestBSBRedLetter|TestNKJV|TestWEBAndWEBC|TestApplePane|TestAndroidPane|TestStyled|TestFyne' .
```

### If you need to change or undo this

- `bsbRedLetterSpansOn` in `red_letter_bsb.go` turns span rendering off for every
  edition in one line; `BIBLETEXT_BSB_RED_LETTER=0|1` overrides it at runtime.
  Off, all four panes return to whole-verse red, which is asserted by a test.
...
- Every table is guarded by the verse's rune length. If a supplier changes the
  text, the guard refuses and the pane falls back rather than colouring
  arbitrary words. That is why a stale table degrades instead of breaking.
