...
- **Static only — the premise was right.** No server-side code, no custom
  ...
  domains, and any build you like in CI *before* deploy. So "all scripting
...
Honest cost: we own a micro-SSG (dev preview, cache-busting, publish script)
...
5. **M5 — polish & review**: visual pass against the app side-by-side, docs
   (ARCHITECTURE/CLAUDE/README), and — with owner approval per the standing
   rule — a landing-page mention.
