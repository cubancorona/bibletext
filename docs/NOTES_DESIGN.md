# Historical notes design — superseded

> **Superseded by [`NOTES_SPEC.md`](NOTES_SPEC.md).** This historical design
> assumed at most one note per chapter per translation and no sender metadata.
> It is retained for legacy defect identifiers and staging rationale only. The
> current record, store, anchor, plan, tint, attribution, and spacing contracts
> are defined in the specification.

> **Companion to [`NOTES_STATE.md`](NOTES_STATE.md).** That document records the
> legacy state model and defect identifiers. This document records the proposed
> replacement that preceded the current implementation.
>
> The code sketches below describe a pre-implementation state and are not
> current types.

## 0. Architectural motivation

The legacy scalar model could not express the required set of notes. Several
otherwise-correct fixes therefore exposed adjacent failures in identity,
...
The underlying mismatch was cardinality: `loadNote` returned **one** note from a
store that could hold several. Partial writes to the four-field mirror also
showed that a convention could not keep independently mutable identity fields
coherent.
...
The temporary view rule permits at most one expanded note, including none. It
must be removable without leaving persistent state behind.
...
There are four note-drawing surfaces.
...
2. **Placement-copy granularity.** The derive distinguishes three reasons a
   note has no home here: the book does not exist in this translation, the
   numbering does not line up, or the anchor maps into a different chapter.
   Separate copy is more precise; consolidated copy is quieter.
3. **Focus persistence.** This design keeps focus session-only, so a relaunch
   expands the newest never-closed note. Nothing is hidden and the view cap
   leaves zero store residue. Persistence would require a separate preference,
   not a store-field change.
4. **PARKED notes live only in memory.** A note held in `pendingLink` because the
   ...
   passage the reader has not reached yet. This remains a separate durability
   decision for the arrival path.
