...
| **H4** | web → app ("Open in app") | **Partly built, Batch 2.** Still cannot work as a plain link on iOS: no custom URL scheme is registered on iOS or the Mac — `cmd/mobile/AndroidManifest.xml` declares only `https` app links and there is no `CFBundleURLTypes` in the repo or the packaging scripts (the Windows Store manifest and the Linux desktop entry are where the `bibletext:` scheme lives) — and a Universal Link does **not** open the app when the reader is already in Safari on the same domain. What the notice pages ship instead, with no app change (`cmd/websitegen/notice.go` → `openInApp`, `notice_assets.go` → `noticeJS`): **Android** gets an `intent://…;package=uk.co.bibletext;S.browser_fallback_url=…` link, which really does open the app — but the intent grammar has no room for the target's own fragment, so **the verse and the note do not cross** (I4 still open on that branch). **iOS** gets Apple's Smart App Banner (`<meta name="apple-itunes-app">`, `app-argument` = the canonical chapter URL) plus a button to the App Store product page, whose own button reads OPEN when the app is installed; the page says so out loud rather than pretending. **Windows and Linux** get a `bibletext:` link — the page's own URL with the scheme swapped, so host, path AND fragment cross: the verse and the note arrive (`share_link_argv.go` swaps the scheme back and hands it to `HandleShareLink`) — with a hidden "Get BibleText" link beneath for a machine without the app, since Chrome and Edge show nothing when no handler answers. The Windows Store build also takes `https://bibletext.co.uk/...` links launched through the shell (its web-to-app handler); links clicked inside a browser stay in the browser on every desktop, by design. **The Mac** still gets the download: no scheme is registered there yet (the `openURLs` entry point in `share_link_macos.go` is ready for one). |
...
- **I2 — No claim without an answer.** If the app claims a link it must do something visible; if it cannot handle it, it must *decline* so the OS falls back to the browser. (`B_IOS_SWALLOW` is fixed.) On Windows and Linux an ACTIVATED app has no OS fallback — the process was already launched with the URL — so `deliverStartupLink` and the single-instance listener open a declined URL in the browser themselves, through the loop-safe opener; the echo guard (`share_link_echo.go`) keeps the Store build's handler from catching that hop again.
...
- **Argv and the single-instance listener are new CALLERS of `HandleShareLink`, not new writers of the park.** They deliver on the main goroutine after `CreateMainUI` and before the load (`app.go` `Run`, pinned by `TestRunWiresTheStartupLinkBetweenTheUIAndTheLoad`); the listener must keep re-checking the host (`normaliseSiteURL`) before delivering, and the echo guard must stay in front of both.
