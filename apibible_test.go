...
func TestDecodeAPIBibleChapterPoetryLines(t *testing.T) {
	vs, _, _, err := decodeAPIBibleChapter(json.RawMessage(psalmChapterContent), "Psalms", 46)
	if err != nil {
		t.Fatal(err)
	}
	if len(vs) != 2 {
...
func TestDecodeAPIBibleChapterNestedSpans(t *testing.T) {
	vs, _, _, err := decodeAPIBibleChapter(json.RawMessage(proseChapterContent), "John", 3)
	if err != nil {
		t.Fatal(err)
	}
	if len(vs) != 2 {
...
func TestDecodeAPIBibleChapterRejectsNonJSONContent(t *testing.T) {
	...
	if _, _, _, err := decodeAPIBibleChapter(json.RawMessage(`"<p>html soup</p>"`), "John", 3); err == nil {
		t.Fatal("string content must be rejected")
	}
	if _, _, _, err := decodeAPIBibleChapter(json.RawMessage(`[]`), "John", 3); err == nil {
...
func TestDecodeAPIBiblePassageMultiChapter(t *testing.T) {
	...
	m, _, _, err := decodeAPIBiblePassage(json.RawMessage(content), "Genesis", 7)
	if err != nil {
		t.Fatal(err)
	}
