...
and the **web reader** (`/web/…`, `/bsb/…`, `/webc/…` — where most of the app's
"Share as link" URLs land) is generated by `cmd/websitegen`. GitHub Pages serves
the **`gh-pages` branch root**, so editing `docs/` on main does not change the
live site. Note the asymmetry since NKJV links shipped: the app can also emit
`/nkjv/…`, and the site publishes nothing there (see the URL contract below).
...
drift. Pure stdlib `html/template`, no npm, no framework. **5,470 static files,
43.7 MB** (measured 2026-08-14: 3,906 pages of scripture + 1,550 pages that name
a passage without showing it + 2 shared asset pairs, the fonts and the 404),
built in seconds; translation JSON is cached under `build/` (gitignored) so
...
with CSS `:target` and zero JS), and an unrecognised version id falls back to
`web`. The reader sits at the ROOT (no `/read/` prefix) to keep shared links
short, which means it SHARES the root namespace with the hand-written pages: the
version ids and `/assets/` are reserved there forever, and `cmd/websitegen`
refuses to write `index.html`, `privacy.html`, `support.html` or `CNAME`
(reservedRootNames) so a generator change can never clobber the live landing or
App Store pages.

**Two sets, not one** (`share_link.go`). `linkPathVersionIDs` — what a URL may
say — is `web`, `bsb`, `webc` AND `nkjv`. `webPublishedVersionIDs` — what the
site serves — is still only the public-domain three, and `cmd/websitegen` keeps
its own hardcoded copy guarded by `licensed_exclusion_test.go`. They were one
name until the owner asked for NKJV links that "work just like the others"; the
old rule (a licensed id must never appear in a URL) was mis-stated, because a
licence covers the TEXT and not the name of a translation.

**`/nkjv/…` NOW RESOLVES, AND STILL PUBLISHES NO TEXT.** `bibletext.co.uk/nkjv/john/3/`
used to be a live 404, so an NKJV link was dead for every recipient without the
app and its Open Graph preview was a bare URL in every messenger. All 1,189
chapters (plus the version and book indexes) are now generated by
`cmd/websitegen/notice.go` as **notice pages**: they name the passage, say the
text isn't published here, carry an explicit open-in-app affordance, and offer
the same passage in the three published translations. **Not one word of the NKJV
is on them** — not in the body, not in `og:description`, not in the title beyond
the reference. Owner framing: *"a licensing message and link to app download for
all NKJV … this is temporary to get things stable and clean for now."*

The same renderer fills the CANON GAPS inside the published translations — the
seven deuterocanonical books' 137 chapters and the Greek Daniel's two extra ones
(`/web/tobit/1/`, `/web/daniel/13/`), which `writeVersion` used to skip into a
404. Same page, same job: name it, say why, offer the translation that has it.

The two lists stay apart: `noticedVersions()` carries an id and a display name,
`publishedVersions()` carries a decoder and a URL to fetch scripture from. Do not
publish the NKJV's TEXT by widening `webPublishedVersionIDs`, adding it to
`publishedVersions`, or relaxing `licensed_exclusion_test.go` — that is a
deliberate act with a licence behind it (© 1982 Thomas Nelson, via API.Bible), an
attribution line it must carry, and an owner decision recorded against it.
The exclusion test no longer bans the substring "nkjv" (the pages have to say
"New King James Version"); it now bans SCRIPTURE under a noticed root, proved
with a sentinel token in the fixture verses, and still bans any mention of a
licensed translation on the pages that do carry text.

**The three published trees are byte-identical across this change**, and must
stay that way: `reader.css`/`reader.js` are content-hashed into the `<link>` of
all 3,906 scripture pages, so the notice pages carry their own
`notice.css`/`notice.js` rather than adding a rule to the reader's.
